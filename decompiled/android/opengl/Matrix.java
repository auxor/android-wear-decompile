...
public class Matrix {
    ...
    public static boolean invertM(float[] mInv, int mInvOffset, float[] m, int mOffset) {
        ...
        float invdet = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / det;
    ...
    public static void orthoM(float[] m, int mOffset, float left, float right, float bottom, float top, float near, float far) {
        if (left == right) {
            ...
            float r_width = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / (right - left);
            float r_height = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / (top - bottom);
            float r_depth = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / (far - near);
            ...
            m[mOffset + 13] = ty;
            m[mOffset + 14] = tz;
            m[mOffset + 15] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
            m[mOffset + 1] = 0.0f;
            m[mOffset + 2] = 0.0f;
            m[mOffset + 3] = 0.0f;
            m[mOffset + 4] = 0.0f;
    ...
    public static void frustumM(float[] m, int offset, float left, float right, float bottom, float top, float near, float far) {
        if (left == right) {
            ...
            float r_width = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / (right - left);
            float r_height = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / (top - bottom);
            float r_depth = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / (near - far);
            ...
            m[offset + 10] = C;
            m[offset + 14] = D;
            m[offset + 11] = LayoutParams.BRIGHTNESS_OVERRIDE_NONE;
            m[offset + 1] = 0.0f;
            m[offset + 2] = 0.0f;
            m[offset + 3] = 0.0f;
            m[offset + 4] = 0.0f;
    ...
    public static void perspectiveM(float[] m, int offset, float fovy, float aspect, float zNear, float zFar) {
        float f = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / ((float) Math.tan(((double) fovy) * 0.008726646259971648d));
        float rangeReciprocal = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / (zNear - zFar);
        m[offset + 0] = f / aspect;
        m[offset + 1] = 0.0f;
        m[offset + 2] = 0.0f;
        m[offset + 3] = 0.0f;
    ...
    public static void setIdentityM(float[] sm, int smOffset) {
        ...
        for (i = 0; i < 16; i += 5) {
            sm[smOffset + i] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
        }
    }
    ...
    public static void setRotateM(float[] rm, int rmOffset, float a, float x, float y, float z) {
        ...
        rm[rmOffset + 13] = 0.0f;
        rm[rmOffset + 14] = 0.0f;
        rm[rmOffset + 15] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
        a *= 0.017453292f;
        float s = (float) Math.sin((double) a);
        float c = (float) Math.cos((double) a);
        if (LayoutParams.BRIGHTNESS_OVERRIDE_FULL == x && 0.0f == y && 0.0f == z) {
            rm[rmOffset + 5] = c;
            rm[rmOffset + 10] = c;
            rm[rmOffset + 6] = s;
            rm[rmOffset + 9] = -s;
            ...
            rm[rmOffset + 4] = 0.0f;
            rm[rmOffset + 8] = 0.0f;
            rm[rmOffset + 0] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
        } else if (0.0f == x && LayoutParams.BRIGHTNESS_OVERRIDE_FULL == y && 0.0f == z) {
            rm[rmOffset + 0] = c;
            rm[rmOffset + 10] = c;
            rm[rmOffset + 8] = s;
            rm[rmOffset + 2] = -s;
            ...
            rm[rmOffset + 6] = 0.0f;
            rm[rmOffset + 9] = 0.0f;
            rm[rmOffset + 5] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
        } else if (0.0f == x && 0.0f == y && LayoutParams.BRIGHTNESS_OVERRIDE_FULL == z) {
            rm[rmOffset + 0] = c;
            rm[rmOffset + 5] = c;
            rm[rmOffset + 1] = s;
            rm[rmOffset + 4] = -s;
            ...
            rm[rmOffset + 8] = 0.0f;
            rm[rmOffset + 9] = 0.0f;
            rm[rmOffset + 10] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
        } else {
            float len = length(x, y, z);
            if (LayoutParams.BRIGHTNESS_OVERRIDE_FULL != len) {
    ...
    public static void setLookAtM(float[] rm, int rmOffset, float eyeX, float eyeY, float eyeZ, float centerX, float centerY, float centerZ, float upX, float upY, float upZ) {
        ...
        float rlf = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / length(fx, fy, fz);
        fx *= rlf;
        fy *= rlf;
        fz *= rlf;
        ...
        float rls = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / length(sx, sy, sz);
        sx *= rls;
        sy *= rls;
        sz *= rls;
