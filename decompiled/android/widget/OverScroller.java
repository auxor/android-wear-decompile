...
public class OverScroller {
    ...
    static class SplineOverScroller {
        ...
        static {
            ...
            float x_min = 0.0f;
            float y_min = 0.0f;
            for (int i = SPLINE; i < NB_SAMPLES; i += CUBIC) {
                float x;
                float coef;
                float y;
        ...
        private void adjustDuration(int start, int oldFinal, int newFinal) {
            ...
            int index = (int) (SensorManager.LIGHT_CLOUDY * x);
            if (index < NB_SAMPLES) {
                float x_inf = ((float) index) / SensorManager.LIGHT_CLOUDY;
                float x_sup = ((float) (index + CUBIC)) / SensorManager.LIGHT_CLOUDY;
                float t_inf = SPLINE_TIME[index];
                this.mDuration = (int) (((float) this.mDuration) * (t_inf + (((x - x_inf) / (x_sup - x_inf)) * (SPLINE_TIME[index + CUBIC] - t_inf))));
            }
        }
        ...
        boolean springback(int start, int min, int max) {
            ...
            this.mVelocity = SPLINE;
            this.mStartTime = AnimationUtils.currentAnimationTimeMillis();
            this.mDuration = SPLINE;
            if (start < min) {
                startSpringback(start, min, SPLINE);
            } else if (start > max) {
                startSpringback(start, max, SPLINE);
            }
            if (this.mFinished) {
                return false;
            }
        ...
        boolean continueWhenFinished() {
            switch (this.mState) {
                ...
                case BALLISTIC /*2*/:
                    this.mStartTime += (long) this.mDuration;
                    startSpringback(this.mFinal, this.mStart, SPLINE);
                    break;
            }
            update();
            return true;
                ...
                switch (this.mState) {
                    case SPLINE /*0*/:
                        ...
                        if (index < NB_SAMPLES) {
                            ...
                            float t_sup = ((float) (index + CUBIC)) / SensorManager.LIGHT_CLOUDY;
                            float d_inf = SPLINE_POSITION[index];
                            velocityCoef = (SPLINE_POSITION[index + CUBIC] - d_inf) / (t_sup - t_inf);
                            distanceCoef = d_inf + ((t - t_inf) * velocityCoef);
                        }
                        distance = (double) (((float) this.mSplineDistance) * distanceCoef);
                        float f = (float) this.mSplineDistance;
                        this.mCurrVelocity = ((r0 * velocityCoef) / ((float) this.mSplineDuration)) * 1000.0f;
                        break;
                    case CUBIC /*1*/:
