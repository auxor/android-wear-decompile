...
public class DdmHandleHello extends ChunkHandler {
    ...
    static {
        CHUNK_HELO = type("HELO");
        CHUNK_WAIT = type("WAIT");
    ...
    private Chunk handleHELO(Chunk request) {
        ...
        ByteBuffer out = ByteBuffer.allocate(((((vmIdent.length() * 2) + 28) + (appName.length() * 2)) + (instructionSetDescription.length() * 2)) + (vmFlags.length() * 2));
