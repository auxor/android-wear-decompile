...
public class PathParser {
    ...
    public static class PathDataNode {
        ...
        public static void nodesToPath(PathDataNode[] node, Path path) {
            ...
            char previousCommand = DateFormat.MINUTE;
        ...
        public void interpolatePathDataNode(PathDataNode nodeFrom, PathDataNode nodeTo, float fraction) {
            for (int i = 0; i < nodeFrom.mParams.length; i++) {
                this.mParams[i] = (nodeFrom.mParams[i] * (RemoteControlClient.PLAYBACK_SPEED_1X - fraction)) + (nodeTo.mParams[i] * fraction);
            }
        }
        ...
        private static void addCommand(Path path, float[] current, char previousCmd, char cmd, float[] val) {
            ...
            switch (cmd) {
                ...
                case RILConstants.RIL_REQUEST_NV_READ_ITEM /*118*/:
                    incr = 1;
                    break;
                ...
                case RILConstants.RIL_REQUEST_VOICE_RADIO_TECH /*108*/:
                ...
                case RILConstants.RIL_REQUEST_IMS_SEND_SMS /*113*/:
        ...
        private static void arcToBezier(Path p, double cx, double cy, double a, double b, double e1x, double e1y, double theta, double start, double sweep) {
            int numSegments = Math.abs((int) Math.ceil((4.0d * sweep) / 3.141592653589793d));
            double eta1 = start;
    ...
    static {
        LOGTAG = PathParser.class.getSimpleName();
    }
    ...
    private static int nextStart(String s, int end) {
        while (end < s.length()) {
            char c = s.charAt(end);
            if (((c - 65) * (c - 90) <= 0 || (c - 97) * (c - 122) <= 0) && c != 'e' && c != DateFormat.DAY) {
                break;
            }
            end++;
        }
    ...
    private static float[] getFloats(String s) {
        int i = 1;
        int i2 = s.charAt(0) == DateFormat.TIME_ZONE ? 1 : 0;
        if (s.charAt(0) != 'Z') {
            i = 0;
        }
        if ((i2 | i) != 0) {
    ...
    private static void extract(String s, int start, ExtractFloatResult result) {
        ...
        for (currentIndex = start; currentIndex < s.length(); currentIndex++) {
            ...
            switch (s.charAt(currentIndex)) {
                ...
                case RILConstants.RIL_REQUEST_SET_SMSC_ADDRESS /*101*/:
                    isExponential = true;
                    break;
            }
            if (foundSeparator) {
