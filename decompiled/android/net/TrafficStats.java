...
public class TrafficStats {
    ...
    public static long getMobileTxPackets() {
        ...
        int len$ = arr$.length;
        for (int i$ = TYPE_RX_BYTES; i$ < len$; i$ += TYPE_RX_PACKETS) {
            total += getTxPackets(arr$[i$]);
        }
        return total;
    }
    ...
    public static long getMobileTcpRxPackets() {
        ...
        int len$ = arr$.length;
        for (int i$ = TYPE_RX_BYTES; i$ < len$; i$ += TYPE_RX_PACKETS) {
            long stat = nativeGetIfaceStat(arr$[i$], TYPE_TCP_RX_PACKETS);
            if (stat != -1) {
                total += stat;
            }
        }
    ...
    public static long getMobileTcpTxPackets() {
        ...
        int len$ = arr$.length;
        for (int i$ = TYPE_RX_BYTES; i$ < len$; i$ += TYPE_RX_PACKETS) {
            long stat = nativeGetIfaceStat(arr$[i$], TYPE_TCP_TX_PACKETS);
            if (stat != -1) {
                total += stat;
            }
        }
