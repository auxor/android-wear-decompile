...
public final class IoUtils {
    private static final Random TEMPORARY_DIRECTORY_PRNG;
            ...
            try {
                ...
                try {
                    ...
                    if (capacity == 0) {
                        ...
                        capacity = DateIntervalFormat.FORMAT_UTC;
                    }
    ...
    public static void setBlocking(FileDescriptor fd, boolean blocking) throws IOException {
        try {
            ...
            if (blocking) {
                flags &= OsConstants.O_NONBLOCK ^ -1;
            } else {
                flags |= OsConstants.O_NONBLOCK;
            }
            Libcore.os.fcntlLong(fd, OsConstants.F_SETFL, (long) flags);
        } catch (ErrnoException errnoException) {
            throw errnoException.rethrowAsIOException();
        }
    }
    ...
    public static File createTemporaryDirectory(String prefix) {
        File result;
        do {
            result = new File(System.getProperty("java.io.tmpdir"), prefix + TEMPORARY_DIRECTORY_PRNG.nextInt());
        } while (!result.mkdir());
        return result;
    }
