    ...
    protected Key engineTranslateKey(Key key) throws InvalidKeyException {
        ...
        if (!((key instanceof OpenSSLRSAPublicKey) || (key instanceof OpenSSLRSAPrivateKey))) {
            if (key instanceof RSAPublicKey) {
                ...
                try {
                    key = engineGeneratePublic(new RSAPublicKeySpec(rsaKey.getModulus(), rsaKey.getPublicExponent()));
                ...
                try {
                    key = engineGeneratePrivate(new RSAPrivateCrtKeySpec(rsaKey2.getModulus(), rsaKey2.getPublicExponent(), rsaKey2.getPrivateExponent(), rsaKey2.getPrimeP(), rsaKey2.getPrimeQ(), rsaKey2.getPrimeExponentP(), rsaKey2.getPrimeExponentQ(), rsaKey2.getCrtCoefficient()));
                ...
                try {
                    key = engineGeneratePrivate(new RSAPrivateKeySpec(rsaKey3.getModulus(), rsaKey3.getPrivateExponent()));
                ...
                try {
                    key = engineGeneratePrivate(new PKCS8EncodedKeySpec(encoded));
                ...
                try {
                    key = engineGeneratePublic(new X509EncodedKeySpec(encoded));
