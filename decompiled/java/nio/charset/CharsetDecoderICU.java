...
final class CharsetDecoderICU extends CharsetDecoder {
    ...
    public static CharsetDecoderICU newInstance(Charset cs, String icuCanonicalName) {
        ...
        try {
            ...
            CharsetDecoderICU result = new CharsetDecoderICU(cs, NativeConverter.getAveCharsPerByte(j), j);
            j = 0;
    ...
    private void setPosition(CharBuffer out) {
        if (out.hasArray()) {
            out.position((out.position() + this.data[OUTPUT_OFFSET]) - out.arrayOffset());
        } else {
            out.put(this.output, (int) INPUT_OFFSET, this.data[OUTPUT_OFFSET]);
        }
        this.output = null;
    }
