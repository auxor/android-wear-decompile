...
public class BerOutputStream {
    ...
    public final void encodeTag(int tag) {
        ...
        this.offset = i + 1;
        bArr[i] = (byte) tag;
        if (this.length > Float.MAX_EXPONENT) {
            int eLen;
            byte numOctets = (byte) 1;
            for (eLen = this.length >> 8; eLen > 0; eLen >>= 8) {
                numOctets = (byte) (numOctets + 1);
            }
            this.encoded[this.offset] = (byte) (numOctets | NodeFilter.SHOW_COMMENT);
            this.offset++;
            eLen = this.length;
    ...
    public void encodeOID() {
        ...
        while (i > 1) {
            elem = oid[i];
            if (elem > Float.MAX_EXPONENT) {
                this.encoded[(this.offset + oidLen) - 1] = (byte) (elem & Float.MAX_EXPONENT);
                for (elem >>= 7; elem > 0; elem >>= 7) {
                    oidLen--;
                    this.encoded[(this.offset + oidLen) - 1] = (byte) (elem | NodeFilter.SHOW_COMMENT);
                }
            } else {
